...
import * as TWEEN from "../../libs/tween.js-master/dist/tween.esm.js";
...
export function getPlane({
  ...
  const alphaRate = 0.03;
  const hueRateOffset = index * 0.001;
  const hueRate = 0.01 + hueRateOffset;
  let goalScale = 1 - index * multiplier + multiplier * 0.1;
  let goalAlpha = 0.2;
  let currentHue = 0.0;
  let goalHue = startHue;
  ...
  function transition({
    ...
    colorTween = new TWEEN.Tween(material.color.getHSL(col))
      ...
      .to({ h: startHue, s: saturation, l: lightness }, duration)
    ...
    if (newNumPlanes !== numPlanes) {
      ...
      goalScale = 1 - index * multiplier + multiplier * 0.1;
    }
  }
